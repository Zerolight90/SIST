...
public class Ex2_Frame extends JFrame {
	...
	//스페이스바를 누를 때마다 생성되는 총알객체(thread)를 저장할 ArrayList!
	...
	public Ex2_Frame() {
		this.add(p = new JPanel() {
			...
			protected void paintComponent(Graphics g) {
				// 더블버퍼링 기법 - 현 JPanel과 같은 크기의 Image객체를 준비하자
				...
				for(int i=0; i<al.size(); i++) {
					//총알객체를 ArrayList로부터 하나씩 얻어내어 b라는 변수에 저장하자
		...
		// 상자의 위치를 잡아주기 위해 setVisible이 후에 x,y좌표를 지정해야 한다.
		x = p.getWidth()/2 - w/2;
		y = p.getHeight() - (h+10);
