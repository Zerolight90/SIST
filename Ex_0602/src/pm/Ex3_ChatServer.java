...
public class Ex3_ChatServer {
		...
		public void run() {
			...
			while(true) {
				...
				try {
					...
					//Ex2_chatServer에 있는 함수들을 호출하기 위해
					list.add(cc);
	...
	//List에서 특정copyclient객체를 삭제하는 기능
	//삭제하고자 하는 객체의 주소를 인자로 받아야 하낟.
	public void removeClient(Ex3_CopyClient cc) {
		list.remove(cc);
	};
	
	//접속자 모두에세 메세지(Ex3_protocol)를 전달하는 기능
	public void sendProtocol(Ex3_Protocol p) {
		for(int i =0; i<list.size(); i++) {
			//각 클라이언트 복사본들을 하나씩 얻어야 한다.
